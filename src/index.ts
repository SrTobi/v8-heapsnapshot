...
type MetaData = {
  ...
  readonly trace_function_info_fields: [
    "function_id",
    "name",
    "script_name",
    "script_id",
...
namespace Sanity {
  ...
  function assertObject(path: string, x: unknown) {
    assertX(
      `${path} is not an object, but is ${typeof x}`,
      typeof x === "object"
    );
  }
  ...
  function assertArray<T>(path: string, arr: T[]) {
    assertX(
      `${path} is not an array, but is '${typeof arr}'`,
      Array.isArray(arr)
    );
  }
  ...
  function assertInteger(path: string, n: number) {
    assertX(
      `${path} is not an integer, but is '${typeof n}'`,
      Number.isInteger(n)
    );
  }
  ...
  function checkMetaData<T>(
    ...
    if (proto instanceof Optional) {
      ...
      if (!Array.isArray(data)) {
        console.warn(
          `Expected '${path}' to be an array but was ${JSON.stringify(data)}!`
        );
        return false;
      }
      if (data.length > proto.length) {
        console.warn(
          `Array at '${path}' has ${data.length - proto.length} new element!`
        );
      ...
      if (Array.isArray(data) || typeof data !== "object") {
        console.warn(
          `Expected '${path}' to be an object but was ${JSON.stringify(data)}!`
        );
        return false;
      }
  ...
  export function check(data: RawSnapshotData) {
    ...
    assertInteger(
      "data.snapshot.trace_function_count",
      snapshot.trace_function_count
    );
    ...
    assertX(
      `expected at least 6 node fields, but got ${parseInfo.nodeFieldCount}`,
      parseInfo.nodeFieldCount >= 6
    );
    ...
    if (!ok) {
      console.error(
        "Heapsnapshot format changed! Please report to https://github.com/SrTobi/v8-heapsnapshot/issues"
      );
    ...
    assertLength(
      "node elements",
      data.nodes.length,
      snapshot.node_count * meta.node_fields.length
    );
    assertLength(
      "edge elements",
      data.edges.length,
      snapshot.edge_count * meta.edge_fields.length
    );
    assertLength(
...
class NodeImpl implements Node {
  ...
  print(deep: number = 2, indent: number = 0, edge_prefix?: string): void {
    console.log(
      "|" + Array(indent + 1).join("  ") + (edge_prefix || "") + this.toString()
    );
...
class SnapshotImpl implements Snapshot {
  ...
  get modules(): Node[] {
    if (!this._modules) {
      this._modules = this.nodes.filter(
        (node) => node.name === "Module" && node.type === "object"
      );
    }
